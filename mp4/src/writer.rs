...
use std::io::{ Read, Write, Seek, Result, SeekFrom, Cursor };
use byteorder::{ ReadBytesExt, WriteBytesExt, BigEndian };

use crate::{merge::{read_box, has_children, fourcc}, desc_reader::Desc};
...
pub fn rewrite_from_desc<R: Read + Seek, W: Write + Seek>(d: &mut R, output_file: &mut W, desc: &mut Desc, track: usize, max_read: u64) -> Result<u64> {
    ...
    while let Ok((typ, _, size, header_size)) = read_box(d) {
        if size == 0 || typ == 0 { break; }

        total_read_size += size;
        let mut new_size = size;
        if has_children(typ, false) {
            // Copy the header
            ...
            // Merge all mdats
            for (buffer, mo, ms) in &desc.mdat_position {
                if let Some(buffer) = buffer {
                    let mut buffer = Cursor::new(buffer.as_ref());
                    buffer.seek(SeekFrom::Start(*mo))?;
                    std::io::copy(&mut buffer.take(*ms), output_file)?;
                    new_size += ms;
                }
            }
